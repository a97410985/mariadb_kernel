...
from mariadb_kernel.introspection.lexer_caller import LexerTokenInfo
...
class BisonXmlReader:
    ...
    def read(self):
        ...
        if rules_element is not None:
            for rule_element in rules_element:
                ...
                if lhs_element is not None and rhs_element is not None:
                    rule_list.append(
                        {
                            "lhs": [lhs_element.text or ""],
                            "rhs": [
                                element.text or ""
                                for element in list(rhs_element or [])
                            ],
                        }
                    )
        ...
        if terminals_element:
            for terminal_element in terminals_element:
                self.terminal_symbol_list.append(
                    {
                        "name": terminal_element.get("name"),
                        "symbol_number": terminal_element.get("symbol-number"),
                        "token_number": terminal_element.get("token-number"),
                    }
                )
        ...
        if non_terminals_element:
            for non_terminal_element in non_terminals_element:
                non_terminal_symbol_list.append(
                    {
                        "name": non_terminal_element.get("name"),
                        "symbol_number": non_terminal_element.get("symbol-number"),
                    }
                )
                        ...
                        if symbol_name == "$default":
                            ...
                            if rule == "accept":
                                for terminal in self.terminal_symbol_list:
                                    ...
                                    if check_list_item_none(
                                        self.action_table[terminal_name], cur_state_num
                                    ):
                                        insert_fill_none(
                                            self.action_table[terminal_name],
                                            cur_state_num,
                                            "accept",
                                        )
                            else:
    ...
    def translate_tokens_to_bison_internal_num(self, tokens: List[int]):
        ...
        for token in tokens:
            ...
            matches = [
                t
                for t in self.terminal_symbol_list
                if int(t.get("token_number")) == cur_token_num
            ]
            if len(matches) > 0:
    ...
    def translate_fancy_tokens_token_num_to_bison_str(
        self, fancy_tokens: List[LexerTokenInfo]
    ):
        ...
        for index, fancy_token in enumerate(fancy_tokens):
            translated_fancy_token: TranslatedLexerTokenInfoWithMatchRule = (
                TranslatedLexerTokenInfoWithMatchRule(
                    text=fancy_token.get("text"),
                    token_str=translate_tokens[index],
                    first_line=fancy_token.get("first_line"),
                    first_column=fancy_token.get("first_column"),
                    last_line=fancy_token.get("last_line"),
                    last_column=fancy_token.get("last_column"),
                    match_rule=-1,
                    match_index=-1,
                )
            )
