...
from mariadb_kernel.introspection.name_resolution_utils import (
    get_type_of_name_token_by_match_rule,
)
...
class AutocompletionEngine:
    def __init__(
        ...
        self.stack: List[Union[int, TranslatedLexerTokenInfoWithMatchRule]] = [0]
    ...
    def reduce(self, cur_action: str):
        ...
        fancy_reduced_symbol = TranslatedLexerTokenInfoWithMatchRule(
            text="",
            token_str=reduced_symbol,
            first_line=0,
            first_column=0,
            last_line=0,
            last_column=0,
    ...
    def get_introspection_list(self):
        ...
        result = filter(
            lambda t: t["token_str"]
            in ["NAME", "FCOUNT", "FSUBSTRING", "FDATE_ADD", "FDATE_SUB"],
            self.final_tokens,
        )
        for token in list(result):
            name_resolution_list.append(
                WordInfo(
                    text=token["text"],
                    token_str=token["token_str"],
                    first_line=token["first_line"],
                    first_column=token["first_column"],
                    last_line=token["last_line"],
                    last_column=token["last_column"],
                    match_rule=token["match_rule"],
                    match_index=token["match_index"],
                    kind=get_type_of_name_token_by_match_rule(
                        token, token["match_rule"], token["match_index"]
                    ),
                )
            )
